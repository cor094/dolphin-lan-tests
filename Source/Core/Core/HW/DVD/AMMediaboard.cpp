...
// Triforce runs a 32-bit VxWorks environment, where timeval contains two 32-bit longs.
struct TimeVal
{
  u32 seconds;
  u32 microseconds;
};
static_assert(sizeof(TimeVal) == 8);
...
struct PendingSelectCommand
{
  bool active = false;
  bool infinite_timeout = false;
  u32 nfds = 0;
  u32 readfds_offset = 0;
  u32 writefds_offset = 0;
  u32 exceptfds_offset = 0;
  u64 deadline_ticks = 0;
  GuestFdSet readfds{};
  GuestFdSet writefds{};
  GuestFdSet exceptfds{};
...
static CoreTiming::EventType* s_et_select_retry = nullptr;
static PendingSelectCommand s_pending_select{};

// Polling once per millisecond is responsive enough for a local cabinet link while avoiding
// thousands of guest-side network retries per rendered frame. The callback runs in emulated time,
// so it never blocks Dolphin's CPU thread on a host socket wait.
static constexpr u64 SELECT_RETRY_INTERVAL_US = 1'000;
...
static bool IsMarioKartGP()
{
  const u32 game_type = GetGameType();
  return game_type == MarioKartGP || game_type == MarioKartGP2;
}

static s64 MicrosecondsToTicks(Core::System& system, u64 microseconds)
{
  const u64 ticks_per_second = system.GetSystemTimers().GetTicksPerSecond();
  const u64 whole_seconds = microseconds / 1'000'000;
  const u64 remaining_microseconds = microseconds % 1'000'000;

  const u64 max_ticks = std::numeric_limits<u64>::max();
  const u64 seconds_ticks = whole_seconds > max_ticks / ticks_per_second ?
                                max_ticks :
                                whole_seconds * ticks_per_second;
  const u64 subsecond_ticks =
      (remaining_microseconds * ticks_per_second + 999'999) / 1'000'000;
  const u64 total_ticks = seconds_ticks > max_ticks - subsecond_ticks ?
                              max_ticks :
                              seconds_ticks + subsecond_ticks;

  return std::max<s64>(
      1, static_cast<s64>(
             std::min<u64>(total_ticks, static_cast<u64>(std::numeric_limits<s64>::max()))));
}

static void SelectRetryCallback(Core::System& system, u64, s64 cycles_late);
...
void Init()
{
  ...
  s_et_select_retry =
      core_timing.RegisterEvent("AMMediaboardSelectRetry", SelectRetryCallback);
...
static bool ReadGuestFdSet(u32 guest_fds_offset, GuestFdSet* guest_fds)
{
  *guest_fds = {};
  if (guest_fds_offset == 0)
    return true;
...
static std::vector<WSAPOLLFD> BuildPollFds(u32 nfds, const GuestFdSet& readfds,
                                           const GuestFdSet& writefds,
                                           const GuestFdSet& exceptfds)
{
  nfds = std::min<u32>(nfds, std::size(s_sockets));
  std::vector<WSAPOLLFD> pollfds(nfds, WSAPOLLFD{.fd = INVALID_SOCKET});

  for (u32 index = 0; index < nfds; ++index)
  {
    const auto guest_socket = GuestSocket(index);
    auto& pfd = pollfds[index];

    if (readfds.IsFdSet(guest_socket))
      pfd.events |= POLLIN;
    if (writefds.IsFdSet(guest_socket))
      pfd.events |= POLLOUT;
    if (exceptfds.IsFdSet(guest_socket))
      pfd.events |= POLLPRI;
...
static void CompletePendingSelect(int ret, std::span<const WSAPOLLFD> pollfds)
{
  if (!s_pending_select.active)
    return;

  if (ret >= 0)
  {
    WriteGuestFdSetFromPollFds(s_pending_select.readfds_offset, pollfds, POLLIN);
    WriteGuestFdSetFromPollFds(s_pending_select.writefds_offset, pollfds, POLLOUT);
    WriteGuestFdSetFromPollFds(s_pending_select.exceptfds_offset, pollfds, POLLPRI);
  }

  auto response = s_pending_select.response;
  reinterpret_cast<u8*>(response.data())[1] = 0;
  response[1] = static_cast<u32>(ret);
  reinterpret_cast<u8*>(response.data())[3] |= 0x80;  // Command complete flag

  s_pending_select.active = false;
  s_exec2_last_response = response;
...
static void SelectRetryCallback(Core::System& system, u64, s64 cycles_late)
{
  ...
  auto pollfds = BuildPollFds(s_pending_select.nfds, s_pending_select.readfds,
                              s_pending_select.writefds, s_pending_select.exceptfds);
  const int ret = PlatformPoll(pollfds, std::chrono::milliseconds{0});

  if (ret < 0)
  {
    s_last_error = SOCKET_ERROR;
    ERROR_LOG_FMT(AMMEDIABOARD_NET, "GC-AM: asynchronous select failed: {}",
                  Common::StrNetworkError());
    CompletePendingSelect(ret, pollfds);
    return;
  }

  const bool timed_out = !s_pending_select.infinite_timeout &&
                         system.GetCoreTiming().GetTicks() >= s_pending_select.deadline_ticks;
  if (ret == 0 && !timed_out)
  {
    const s64 interval_ticks = MicrosecondsToTicks(system, SELECT_RETRY_INTERVAL_US);
    system.GetCoreTiming().ScheduleEvent(std::max<s64>(1, interval_ticks - cycles_late),
                                         s_et_select_retry);
    return;
  }

  DEBUG_LOG_FMT(AMMEDIABOARD_NET, "GC-AM: asynchronous select result: {}{}", ret,
                timed_out && ret == 0 ? " (timeout)" : "");
  CompletePendingSelect(ret, pollfds);
}

// Returns true when the result is ready immediately. Returns false when the Execute2 response is
// intentionally left pending and will be completed by SelectRetryCallback.
static bool AMMBCommandSelectAsyncExecute2(Core::System& system, u32 parameter_offset)
{
  PendingSelectCommand pending{};
  pending.nfds = std::min<u32>(s_media_buffer_32[parameter_offset], std::size(s_sockets));
  pending.readfds_offset = s_media_buffer_32[parameter_offset + 1];
  pending.writefds_offset = s_media_buffer_32[parameter_offset + 2];
  pending.exceptfds_offset = s_media_buffer_32[parameter_offset + 3];
  const u32 timeout_offset = s_media_buffer_32[parameter_offset + 4];

  if (!ReadGuestFdSet(pending.readfds_offset, &pending.readfds) ||
      !ReadGuestFdSet(pending.writefds_offset, &pending.writefds) ||
      !ReadGuestFdSet(pending.exceptfds_offset, &pending.exceptfds))
  {
    s_media_buffer[1] = 0;
    s_media_buffer_32[1] = SOCKET_ERROR;
    s_last_error = SSC_EFAULT;
    return true;
  }

  u64 timeout_us = 0;
  pending.infinite_timeout = timeout_offset == 0;
  if (!pending.infinite_timeout)
  {
    const auto timeout_span = GetSpanForMediaboardAddress(timeout_offset);
    if (timeout_span.size() < sizeof(TimeVal))
    {
    ...
    TimeVal timeout{};
    std::memcpy(&timeout, timeout_span.data(), sizeof(timeout));
    timeout_us = u64(timeout.seconds) * 1'000'000 + timeout.microseconds;
  }

  auto pollfds = BuildPollFds(pending.nfds, pending.readfds, pending.writefds, pending.exceptfds);
  const int ret = PlatformPoll(pollfds, std::chrono::milliseconds{0});

  if (ret != 0 || (!pending.infinite_timeout && timeout_us == 0))
  {
    if (ret >= 0)
    {
      WriteGuestFdSetFromPollFds(pending.readfds_offset, pollfds, POLLIN);
      WriteGuestFdSetFromPollFds(pending.writefds_offset, pollfds, POLLOUT);
      WriteGuestFdSetFromPollFds(pending.exceptfds_offset, pollfds, POLLPRI);
    }
    else
    {
      s_last_error = SOCKET_ERROR;
  ...
  {
    ERROR_LOG_FMT(AMMEDIABOARD,
                  "GC-AM: Execute2 select submitted while another select was pending");
    s_media_buffer[1] = 0;
    s_media_buffer_32[1] = SOCKET_ERROR;
    s_last_error = SSC_EWOULDBLOCK;
    return true;
  ...
  std::copy_n(s_media_buffer_32.begin(), pending.response.size(), pending.response.begin());
  reinterpret_cast<u8*>(pending.response.data())[1] = 0;
  reinterpret_cast<u8*>(pending.response.data())[3] &= ~0x80;
  pending.response[1] = 0;

  if (!pending.infinite_timeout)
  {
    const u64 now = system.GetCoreTiming().GetTicks();
    const u64 timeout_ticks = static_cast<u64>(MicrosecondsToTicks(system, timeout_us));
    pending.deadline_ticks = now > std::numeric_limits<u64>::max() - timeout_ticks ?
                                 std::numeric_limits<u64>::max() :
                                 now + timeout_ticks;
  }

  s_pending_select = pending;
  // Make DMA reads show the new command as pending instead of exposing the previous completed
  // Execute2 response while the socket wait is in progress.
  s_exec2_last_response = pending.response;

  system.GetCoreTiming().RemoveEvent(s_et_select_retry);
  system.GetCoreTiming().ScheduleEvent(MicrosecondsToTicks(system, SELECT_RETRY_INTERVAL_US),
                                       s_et_select_retry);
  return false;
}
  ...
  case AMMBDICommand::Read:
    ...
    if (offset == DIMMCommandExecute2)
    {
      ...
      case AMMBCommand::Select:
        if (IsMarioKartGP() && !AMMBCommandSelectAsyncExecute2(system, 2))
        {
          // The DI transfer is complete, but the separate AM-Mediaboard keeps this network RPC
          // pending until the socket is ready or the guest timeout expires.
          memory.Memset(address, 0, length);
          return 0;
        }
        break;
...
void Shutdown()
{
  ...
  auto& core_timing = Core::System::GetInstance().GetCoreTiming();
  if (s_et_select_retry != nullptr)
    core_timing.RemoveEvent(s_et_select_retry);
  s_pending_select = {};
...
void DoState(PointerWrap& p)
{
  ...
  p.Do(s_pending_select.active);
  p.Do(s_pending_select.infinite_timeout);
  p.Do(s_pending_select.nfds);
  p.Do(s_pending_select.readfds_offset);
  p.Do(s_pending_select.writefds_offset);
  p.Do(s_pending_select.exceptfds_offset);
  p.Do(s_pending_select.deadline_ticks);
  p.Do(s_pending_select.readfds.bits);
  p.Do(s_pending_select.writefds.bits);
  p.Do(s_pending_select.exceptfds.bits);
  p.Do(s_pending_select.response);
